...
from numpy.random import Generator, MT19937, SeedSequence, Philox
...
class SaDE:
    def __init__(self, EVALUATION_FUNCTION, const_LP=50, POP_SIZE=150, MAX_GEN=2500, PARALLEL=False, SEED=None, SAVE_PATH="", PARALLEL_MAP_FUNCTION=None, BOUNDS=[], NUM_PAR=0):
        ...
        if self.NUM_PAR <= 0:
            print("Algorithm needs NUM_PAR > 0. It will fail otherwise!")
            exit(-1)
        
        self.PARALLEL = PARALLEL
        
        if PARALLEL_MAP_FUNCTION:
        ...
        if not BOUNDS:
            ...
            self.BOUNDS = [(1e-5, 9.8e-1)] * self.NUM_PAR
        else:
            self.BOUNDS = BOUNDS
        self.l_bounds = [i[0] for i in self.BOUNDS]
        self.u_bounds = [i[1] for i in self.BOUNDS]
        
        self.HOF_SIZE = int(math.ceil(self.POP_SIZE*self.MAX_GEN*0.15))
    ...
    def run_SaDE(self):
        ...
        for GEN in range(self.MAX_GEN):
            ...
            if GEN % 10 == 0:
                ...
                if self.ITERATIVE_SAVE == True:
                    self.__save_gen(CURRENT_POPULATION, GEN, end_gen-init_gen)
            ...
            if NO_IMP_GEN > STOP_CRITERIA:
                ...
                self.__save_gen(CURRENT_POPULATION, GEN, end_gen-init_gen)
                self.__save_gen(next_gen, GEN+1, 0)
                break
            if math.isclose(PROB_NEW_BEST.fitness.values[0], 0.0, rel_tol=1e-7, abs_tol=1e-7):
                ...
                self.__save_gen(next_gen, GEN+1, 0)
                break
            if math.isclose(self.stats.compile(CURRENT_POPULATION)['std'], 1e-5, rel_tol=1e-5, abs_tol=1e-5):
        ...
        print(f"Duration = {self.END_TIME-self.START_TIME} seconds")
    
    def print_config(self):
        print("\n\n--------------------------------------------------------------------------------------------------------------------------")
        
        print("\t\t\t\t\tSaDE implementation by João Víctor Costa de Oliveira")
        print("Configuration:")
        ...
        print(f"\t HOF size = {self.HOF_SIZE}")
    ...
    def save_config(self):
        ...
        config_file.write(f"\t Parallel = {self.PARALLEL}\n")
        config_file.write(f"\t crm = {self.crm}\n")
        config_file.write(f"\t HOF size = {self.HOF_SIZE}\n")
        config_file.close()
    ...
    # Métodos privados
    def __update_strategy_probabilities(self):
        """
    ...
    def __gen_initial_pop(self):
        ...
        sample = qmc.scale(sample=sample, l_bounds=self.l_bounds, u_bounds=self.u_bounds)
        ...
        self.__print_gen(self.INITIAL_POP)

    def __gen_new_rnd_pop(self, size):
        pass
    def __print_gen(self, pop):
        print("\n\n-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------")
    ...
    def __save_gen(self, pop, gen_num, time):
