'''
Código feito por João Víctor Costa de Oliveira
Rodando no ambiente conda bcim (fornecido no diretório env/) com Python na versão 3.11.11
Foi utilizando o formatador "black" para formatar o código com os padrões da comunidade Python
'''
...
class SaDE:
    def __init__(
        ...
        if not BOUNDS:
            print(
                "Error: BOUNDS argument cannot be initialized as an empty list! Initializing with random values!"
            )
            BOUNDS = [
                (1e-5, 9.5e-1),
                (1e-5, 9.5e-1),
    ...
    def gen_initial_pop(self):
        ...
        qmc.scale(
            sample=sample, l_bounds=self.l_bounds, u_bounds=self.u_bounds
        )  # Garante que as minhas amostras estejam dentro do intervalo necessário
        ...
        for GEN in range(self.MAX_GEN):
            ...
            for IDV in CURRENT_POPULATION:
                ...
                if any(n < 0 for n in offspring_candidate):
                    ...
                    offspring_candidate.fitness.values = self.TOOLBOX.evaluate(
                        offspring_candidate
                    )
            ...
            if PROB_NEW_BEST.fitness.values[0] == (
                CURRENT_BEST.fitness.values[0] + 1.0e-6
            ) or PROB_NEW_BEST.fitness.values[0] == (
                CURRENT_BEST.fitness.values[0] - 1.0e-6
            ):
                NO_IMP_GEN += 1
